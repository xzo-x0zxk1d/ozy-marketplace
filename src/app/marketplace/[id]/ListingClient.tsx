...
import { ArrowLeft, MessageSquare, Eye, Clock, Tag, Zap, Trash2, ExternalLink, Shield, AlertTriangle, Copy, Check, Loader2, Package, Link2, Gamepad2 } from "lucide-react";
      ...
      <div className={styles.inner}>
        <div className={styles.topBar}>
          ...
          <button className={styles.copyLinkBtn} onClick={handleCopyLink}>
            {linkCopied ? <><Check size={14} /> Link Copied!</> : <><Link2 size={14} /> Copy Listing Link</>}
          </button>
        </div>
        ...
        <div className={styles.layout}>
          <div className={styles.imagePanel}>
            ...
            {listing.game && listing.game !== "Not Game-Specific" && (
              <div className={styles.gameBadge}><Gamepad2 size={13} /> {listing.game}</div>
            )}
          </div>
          ...
          <div className={styles.infoPanel}>
            <div className={styles.metaRow}>
              ...
              {listing.negotiable && <span className={styles.negBadge}>Negotiable</span>}
              {listing.tradeOnly && <span className={styles.tradeBadge}>Trade Only</span>}
            </div>
            ...
            {listing.paymentMethods && listing.paymentMethods.length > 0 && (
              <div className={styles.paymentSection}>
                ...
                <div className={styles.paymentMethods}>
                  {listing.paymentMethods.map((m) => <span key={m} className={styles.paymentChip}>{m}</span>)}
                </div>
              </div>
            )}
            ...
            <div className={styles.contactCard}>
              ...
              <p className={styles.contactSub}>All transactions handled through Discord DMs. Message the seller to arrange payment and delivery.</p>
              ...
              <a href="https://discord.com" target="_blank" rel="noopener noreferrer" className={styles.dmBtn}>
                <MessageSquare size={15} />Open Discord & DM {listing.sellerDiscord}<ExternalLink size={13} />
              </a>
            </div>

            <div className={styles.safetyNote}><Shield size={13} /><span>Always verify items before payment. Ozy Marketplace is not responsible for trade outcomes.</span></div>
